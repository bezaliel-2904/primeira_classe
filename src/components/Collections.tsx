          ...
          {COLLECTIONS.map((col, i) => (
            <a key={col.name} href="#contato" className="group relative overflow-hidden rounded-[2px] aspect-[4/3] md:aspect-[5/4] block reveal border border-white/70 shadow-lg shadow-ink-900/5" style={{ transitionDelay: `${i * 0.1}s` }}>
              <img
                ...
                className="w-full h-full object-cover transition-transform duration-1000 ease-out group-hover:scale-[1.045] bg-sky-50"
                loading="lazy"
                onError={(event) => {
              ...
              <div className="absolute inset-x-0 bottom-0 p-7 md:p-9">
                <div className="flex items-end justify-between gap-5">
                  <div>
                    ...
                    <p className="font-sans text-[13px] text-white/80 max-w-sm leading-relaxed mt-3 opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500">{col.description}</p>
                  </div>
                  <span className="shrink-0 w-11 h-11 rounded-full bg-white/90 backdrop-blur-sm flex items-center justify-center text-sky-700 group-hover:bg-sky-700 group-hover:text-white transition-all duration-300"><ArrowRight size={17} className="group-hover:translate-x-0.5 transition-transform" /></span>
                </div>
              </div>
